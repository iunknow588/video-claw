...
from departments.CMO.skills.progress_ui import ProgressUISkill
from departments.CMO.skills.report_ui import ReportUISkill
from departments.COO.services.use_cases.script_to_publish import ScriptToPublishUseCase

PromotionEventCallback = Callable[[dict[str, Any]], Awaitable[None] | None]


class CMOService:
    """External communication leader that owns user-facing chat and status delivery."""

    def __init__(self, session: AsyncSession):
        self.session = session
        self.pipeline_service = ScriptToPublishUseCase(session)
