...
class SkillRuntimeManager:
        ...
        *,
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                token_usage = self._extract_token_usage(input_data, raw_output, output_json)
    ...
    def _resolve_method_name(self, instance: Any) -> str | None:
        for candidate in ("async_execute", "execute", "run", "build_plan", "record"):
    ...
    def _extract_token_usage(
        ...
        if isinstance(raw_output, dict):
            explicit_usage = raw_output.get("token_usage") or raw_output.get("_token_usage")
        if explicit_usage:
