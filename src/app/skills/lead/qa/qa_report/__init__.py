...
class QAReportSkill(BaseSkill):
    ...
    description = "Aggregates QA dimensions into a final quality gate decision and action route."
    parameters_schema = {
        "type": "object",
        "properties": {
            "checks": {"type": "array"},
    ...
    def execute(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        reroute_target = None
        if not qa_passed:
            if any(item.get("dimension") in {"video_quality", "technical_spec"} for item in failed):
                reroute_target = "lead.production"
            else:
                reroute_target = "lead.research_development"
        ...
        recommendation = (
            "质检通过，允许进入发布组。"
            if qa_passed
            else f"质检未通过，建议打回 {reroute_target} 处理后再复检。"
        )
