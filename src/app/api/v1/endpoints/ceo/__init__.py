...
async def chat_with_ceo(
    ...
    """Compatibility endpoint that proxies chat through the CMO service."""

    async def event_stream():
        service = CEOChatService(db)
        ...
        async def produce() -> None:
            try:
                await service.handle_message(data.message, event_callback=publish)
...
async def get_company_status(
    ...
    return await service.get_company_status()
...
async def set_ceo_workflow(
    ...
    service = CEOControlService(db)
    try:
        return await service.set_workflow(data.graph_definition)
...
async def add_workflow_edge(
    ...
    service = CEOControlService(db)
    try:
        return await service.add_edge(data.from_leader, data.to_leader)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def add_workflow_conditional_edge(
    ...
    service = CEOControlService(db)
    try:
        return await service.add_conditional_edge(data.from_leader, data.router_func, data.mapping)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def list_ceo_leaders(
    ...
    service = CEOControlService(db)
    return await service.list_leaders()
...
async def add_ceo_leader(
    ...
    service = CEOControlService(db)
    try:
        return await service.add_leader(data.name, data.config)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def get_ceo_leader_status(
    ...
    service = CEOControlService(db)
    try:
        return await service.get_leader_status(leader_name)
    except ValueError as exc:
...
async def update_ceo_leader(
    ...
    service = CEOControlService(db)
    try:
        return await service.update_leader_config(leader_name, data.config)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def remove_ceo_leader(
    ...
    service = CEOControlService(db)
    try:
        return await service.remove_leader(leader_name)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
async def rollback_ceo_leader(
    ...
    service = CEOControlService(db)
    try:
        return await service.rollback_leader(leader_name, data.version)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def issue_leader_optimize_command(
    ...
    service = CEOControlService(db)
    try:
        return await service.issue_optimize_command(
...
async def request_leader_report(
    ...
    service = CEOControlService(db)
    try:
        return await service.request_leader_report(leader_name)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
async def get_latest_leader_report(
    ...
    service = CEOControlService(db)
    try:
        return await service.get_latest_leader_report(leader_name=leader_name, report_type=report_type)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
async def approve_leader_change(
    ...
    service = CEOControlService(db)
    try:
        return await service.approve_leader_change(leader_name, data.proposal)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def set_leader_budget(
    ...
    service = CEOControlService(db)
    try:
        return await service.set_leader_budget(leader_name, data.token_limit)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def adjust_leader_resources(
    ...
    service = CEOControlService(db)
    try:
        return await service.adjust_resource_allocation(leader_name, data.resource_type, data.amount)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def get_task_progress(
    ...
    service = CEOControlService(db)
    try:
        return await service.get_task_progress(workflow_run_id)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
async def enable_evolution(
    ...
    service = CEOControlService(db)
    return await service.enable_evolution()
...
async def disable_evolution(
    ...
    service = CEOControlService(db)
    return await service.disable_evolution()
...
async def run_evolution_cycle(
    ...
    service = CEOControlService(db)
    return await service.evolution_cycle()
...
async def collect_periodic_reports(
    ...
    service = CEOControlService(db)
    return await service.collect_periodic_reports(cadence=data.cadence)
